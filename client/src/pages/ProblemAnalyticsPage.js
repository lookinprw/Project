...
function ProblemAnalyticsPage() {
  ...
  const getProblemTypeStats = () => {
    const stats = problems.reduce((acc, problem) => {
      const type = problem.problem_type === 'hardware' ? 'ฮาร์ดแวร์' : 'ซอฟต์แวร์';
  ...
  const fetchProblems = async () => {
    try {
      ...
      let filteredData = response.data.data.filter(problem => 
        problem.status_name === 'เสร็จสิ้น' || problem.status_name === 'ชำรุดเสียหาย'
      );
      
      if (filters.status.length > 0) {
        filteredData = filteredData.filter(problem => 
          filters.status.includes(problem.status_name)
        );
      }
    ...
    <DashboardLayout>
      <div className="p-6">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-2xl font-bold">รายการแจ้งปัญหาที่เสร็จสิ้นและชำรุดเสียหาย</h1>
  ...
  <div className="bg-white p-6 rounded-lg shadow">
    <h2 className="text-lg font-semibold mb-4">ห้องที่พบปัญหามากที่สุด</h2>
    <div className="h-64">
      <ResponsiveContainer width="100%" height="100%">
          ...
          <div className="flex flex-wrap gap-4 mb-4">
            <div>
              ...
              <div className="space-x-2">
                {['เสร็จสิ้น', 'ชำรุดเสียหาย'].map(status => (
                  <label key={status} className="inline-flex items-center">
                    ...
                    />
                    <span className="ml-2 text-sm">{status === 'เสร็จสิ้น' ? 'เสร็จสิ้น' : 'ชำรุดเสียหาย'}</span>
                  </label>
                ))}
              </div>
            </div>
                ...
                {['hardware', 'software'].map(type => (
                  <label key={type} className="inline-flex items-center">
                    <input
                      ...
                      className="rounded border-gray-300 text-indigo-600"
                    />
                    <span className="ml-2 text-sm">{type === 'hardware' ? 'ฮาร์ดแวร์' : 'ซอฟต์แวร์'}</span>
                  </label>
                ))}
              </div>
            </div>
              ...
              <tr>
                <th className="w-4 p-4">
                  <input
                    ...
                    onChange={(e) => setSelectedProblems(
                      e.target.checked ? problems.map(p => p.id) : []
                    )}
                    className="rounded border-gray-300 text-indigo-600"
                  />
                </th>
                {["ลำดับ", "วันที่", "อุปกรณ์", "รหัสครุภัณฑ์", "ห้อง", "ปัญหา", "ประเภท", "สถานะ"].map((header) => (
